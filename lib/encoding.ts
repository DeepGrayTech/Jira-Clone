...
export function utf8Encode(str: string): Uint8Array {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (cp < 0x80) {
      ...
      stats.twoByte++;
      console.log("[utf8Encode] 2字节编码:", {
        char: str[i],
        codePoint: cp.toString(16).padStart(4, "0"),
        hex: [0xc0 | (cp >> 6), 0x80 | (cp & 0x3f)]
      ...
      bytes.push(b1, b2, b3);
      stats.threeByte++;
      console.log("[utf8Encode] 3字节编码:", {
        char: str[i],
        codePoint: cp.toString(16).padStart(4, "0"),
        hex: [b1, b2, b3].map((b) => b.toString(16).padStart(2, "0")).join(" "),
      });
    } else {
      ...
      stats.fourByte++;
      console.log("[utf8Encode] 4字节编码(代理对):", {
        codePoint: cp.toString(16).padStart(6, "0"),
        hex: [b1, b2, b3, b4]
          .map((b) => b.toString(16).padStart(2, "0"))
          .join(" "),
      });
  ...
  console.log("[utf8Encode] 编码完成:", {
    ...
    hasMultiByteChars:
      stats.twoByte > 0 || stats.threeByte > 0 || stats.fourByte > 0,
    stats,
  });
  return result;
}
  ...
  try {
    ...
    while (i < bytes.length) {
      ...
      if (b1 < 0x80) {
        ...
        if (i + 1 >= bytes.length) {
          console.warn("[utf8Decode] 2字节序列不完整:", {
            position: i,
            byteCount: bytes.length - i,
          });
          return null;
        }
        const b2 = bytes[i + 1];
        ...
        charCount = 2;
        console.log("[utf8Decode] 2字节解码:", {
          codePoint: codePoint.toString(16).padStart(4, "0"),
          bytes: [b1, b2].map((b) => b.toString(16).padStart(2, "0")).join(" "),
        });
      } else if ((b1 & 0xf0) === 0xe0) {
        // Branch 3: 3-byte sequence
        if (i + 2 >= bytes.length) {
          console.warn("[utf8Decode] 3字节序列不完整:", {
            position: i,
            byteCount: bytes.length - i,
          });
          return null;
        ...
        if (i + 3 >= bytes.length) {
          console.warn("[utf8Decode] 4字节序列不完整:", {
            position: i,
            byteCount: bytes.length - i,
          });
          return null;
        ...
        codePoint =
          ...
          (b4 & 0x3f);
        charCount = 4;
        console.log("[utf8Decode] 4字节解码:", {
          codePoint: codePoint.toString(16).padStart(6, "0"),
          bytes: [b1, b2, b3, b4]
            .map((b) => b.toString(16).padStart(2, "0"))
            .join(" "),
        });
        ...
        console.warn("[utf8Decode] 无效的UTF-8起始字节:", {
          position: i,
          byte: b1.toString(16).padStart(2, "0"),
        });
        return null;
      }
